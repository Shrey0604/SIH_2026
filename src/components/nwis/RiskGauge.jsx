...
const Fact = ({ icon: Icon, label, value, color, testId }) => (
  <div className="rounded-lg border border-nw-line/70 bg-nw-bg/40 px-3 py-2.5 min-w-0" data-testid={testId}>
    <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-[0.12em] text-nw-dim font-display font-semibold">
      <Icon className="w-3 h-3" /> {label}
    </div>
    <div className="text-xs mt-1 leading-snug" style={{ color: color || "#E2E8F0" }}>{value}</div>
  </div>
);
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center">
        <div className="relative w-[250px] h-[200px]" data-testid="radial-risk-gauge">
          {critical && <span className="absolute left-1/2 top-[125px] -translate-x-1/2 -translate-y-1/2 w-[190px] h-[190px] rounded-full border border-nw-crit/60" style={{ animation: "crit-ping 1.4s ease-out infinite" }} />}
          <svg viewBox="0 0 250 215" className="w-full h-full overflow-visible">
            ...
            {[0, 25, 50, 75, 100].map((p) => {
              ...
              return (
                <text key={p} x={x} y={y} fontSize="9" fill="#64748B" textAnchor="middle" dominantBaseline="middle" fontFamily="JetBrains Mono, monospace">
                  {p}
                </text>
              );
            })}
