...
import { ANOMALY_START, CRITICAL_T } from "@/data/scenario";
...
export const Header = ({ current, tick, running, phase, onToggle, onSkip, onReplay }) => (
  <header className="flex items-center gap-6 py-4 border-b border-nw-line/70" data-testid="dashboard-header">
    <div className="flex items-center gap-3 shrink-0" data-testid="nwis-brand-logo">
      ...
      <div>
        ...
        <div className="text-[10px] tracking-[0.14em] uppercase text-nw-muted mt-1 whitespace-nowrap">Nearby Wells Intelligence System</div>
      </div>
    </div>
    <div className="h-9 w-px bg-nw-line hidden md:block" />
    <div className="hidden md:block min-w-0" data-testid="header-active-well-name">
      <div className="eyebrow !text-[10px]">Active well</div>
    ...
    <div className="ml-auto flex items-center gap-3 shrink-0">
      ...
      <span className="font-mono text-xs text-nw-muted tabular hidden sm:inline" data-testid="header-session-clock">
        T+{mmss(tick)}
      </span>
...
export const DemoTimeline = ({ tick, phase }) => {
  ...
  return (
    <div className="flex items-center gap-4 mt-3" data-testid="demo-timeline">
      <span className="eyebrow !text-[10px] whitespace-nowrap">Scripted scenario</span>
      <div className="relative flex-1 h-1.5 rounded-full bg-nw-line/70">
      ...
      <span className="font-mono text-[11px] text-nw-muted whitespace-nowrap tabular" data-testid="demo-timeline-status">
        {status}
      </span>
    </div>
  );
